...
//只有5和6只出现1次，要找出5和6.
//5: 0101 6:0110   ---   5 ^ 6 -> 0011
// 0: 0000  1:0001  2:0010  3:0011  4:0100  5:0101  6:0110
// Group 1: 1, 1, 3, 3, 5  Group 2: 2, 2, 4, 4, 6
void FindNum(int arr[], int len, int* pNum1, int* pNum2)
{
	//1.整体异或,得到5^6的结果
	int tmp = 0;
	for (int i = 0; i < len; i++)// 0 ^ anyNum = anyNum//// -> find result = 5 ^ 6 -> tmp = 0011
	{
		tmp = tmp ^ arr[i];
	}
	...
	for (int i = 0; i < 32; i++)
	{
		if ((tmp >> i) & 1 == 1) //在 tmp 中，如果有一个位置的值是 1，说明这两个数字在这个位置上的值不同（即它们的二进制表示在这一位上不同）。因此，找到这个位置后，我们就可以把数字分成两组，使得这两个只出现一次的数字分别进入不同的组。
		{
			pos = i;
			break;
		}
	}
	//3.再次遍历数组的每个数字，看pos位置是不是0或者1分到两个组
	for (int i = 0; i < len; i++)
	{
		if ((arr[i] >> pos) & 1 == 1)
		{
